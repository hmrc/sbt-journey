...
If you declare a page with `answerType = FileUpload`, **sbt-journey** will generate additional code to support integration with [Upscan](https://github.com/hmrc/upscan-initiate#upscan-user-manual), MDTP's file upload service.

The `FileUpload` answer type can only be supplied as the top-level `answerType` of a page. It cannot be used in the fields of a `model` and it cannot be marked as optional using the `{Option: ..}` syntax.

When it is used, the generated routes for the page will include extra file upload redirect endpoints, the generated controller will call the [Upscan initiate endpoint](https://github.com/hmrc/upscan-initiate#post-upscanv2initiate) to fetch the form's submission URL prior to rendering, and the `initialiseJourneyViews` task will generate a [file upload](https://design-system.service.gov.uk/components/file-upload/) view.

As well as page-specific code for file uploads, an Upscan notification controller will be generated to accept scan result [notifications](https://github.com/hmrc/upscan-initiate#file-processing-outcome-), and a file upload repository will be generated in order to store the results of scanning.

An extra `internal.routes` file will be generated that routes requests to the Upscan notification controller. These routes will not be routed under the service's usual name prefix, so that when the service's name prefix is added to MDTP's public routing, the notification endpoint will remain private.

The data stored as the user's "answer" on file upload pages will be the `UploadId` used to store the file upload metadata in the `file-uploads` collection.
...
Before your application is built via sbt's `compile` task, **sbt-journey** reads the `journey.conf` configuration file and generates various different classes into the `generated/` source folder:
