...
@ImplementedBy(classOf[DefaultAddSupportingDocumentsController])
trait AddSupportingDocumentsBaseController extends FrontendBaseController, I18nSupport {
  def onPageLoad(mode: Mode): Action[AnyContent]
  def onSubmit(mode: Mode): Action[AnyContent]
}

@Singleton
class DefaultAddSupportingDocumentsController @Inject() (
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  navigator: JourneyNavigator,
  sessionRepository: SessionRepository,
  form: AddSupportingDocumentsBaseFormProvider,
  view: views.html.AddSupportingDocumentsView,
  override val controllerComponents: MessagesControllerComponents
)(using ExecutionContext) extends AddSupportingDocumentsBaseController {

  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val submitRoute = journeyRoutes.AddSupportingDocumentsBaseController.onSubmit(mode)
    val page = AddSupportingDocumentsPage
