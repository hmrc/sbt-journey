...
trait AddConfidentialInformationBaseFormProvider {
  def apply(): Form[Choice]
}

class DefaultAddConfidentialInformationFormProvider
  extends AddConfidentialInformationBaseFormProvider
  with Mappings {

  def apply(): Form[Choice] = Form(
    "value" -> enumerable[Choice](
      requiredKey = "addConfidentialInformation.error.required",
      invalidKey = "addConfidentialInformation.error.invalid",
    )
  )
}
