...
import uk.gov.hmrc.fileuploadjourney.forms.AddConfidentialInformationBaseFormProvider
...
import views.html.AddConfidentialInformationView

import scala.concurrent.{ExecutionContext,Future}
import scala.util.{Success,Try}

class DefaultAddConfidentialInformationControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultAddConfidentialInformationFormProvider()
  ...
  private def makeController(
    ...
    formProvider: AddConfidentialInformationBaseFormProvider = mock[AddConfidentialInformationBaseFormProvider],
    view: AddConfidentialInformationView = mock[AddConfidentialInformationView]
  ) = new DefaultAddConfidentialInformationController(
  ...
  "DefaultAddConfidentialInformationController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[AddConfidentialInformationView]
    ...
    val route = journeyRoutes.AddConfidentialInformationBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.AddConfidentialInformationBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: Choice) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(AddConfidentialInformationPage, answer)
    } yield answers
  ...
  "DefaultAddConfidentialInformationController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: Choice) =>
    ...
    val route = journeyRoutes.AddConfidentialInformationBaseController.onSubmit(NormalMode)
