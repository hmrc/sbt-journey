...
import uk.gov.hmrc.fileuploadjourney.forms.ProvideGoodsNameBaseFormProvider
...
import views.html.ProvideGoodsNameView

import scala.concurrent.{ExecutionContext,Future}
import scala.util.{Success,Try}

class DefaultProvideGoodsNameControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultProvideGoodsNameFormProvider()
  ...
  private def makeController(
    ...
    formProvider: ProvideGoodsNameBaseFormProvider = mock[ProvideGoodsNameBaseFormProvider],
    view: ProvideGoodsNameView = mock[ProvideGoodsNameView]
  ) = new DefaultProvideGoodsNameController(
  ...
  "DefaultProvideGoodsNameController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[ProvideGoodsNameView]
    ...
    val route = journeyRoutes.ProvideGoodsNameBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.ProvideGoodsNameBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(ProvideGoodsNamePage, answer)
    } yield answers
  ...
  "DefaultProvideGoodsNameController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.ProvideGoodsNameBaseController.onSubmit(NormalMode)
