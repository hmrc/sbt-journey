...
class DefaultProvideGoodsDescriptionControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultProvideGoodsDescriptionFormProvider()
  ...
  private def makeController(
    ...
    formProvider: ProvideGoodsDescriptionBaseFormProvider = mock[ProvideGoodsDescriptionBaseFormProvider],
    view: ProvideGoodsDescriptionView = mock[ProvideGoodsDescriptionView]
  ) = new DefaultProvideGoodsDescriptionController(
  ...
  "DefaultProvideGoodsDescriptionController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[ProvideGoodsDescriptionView]
    ...
    val route = journeyRoutes.ProvideGoodsDescriptionBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.ProvideGoodsDescriptionBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(ProvideGoodsDescriptionPage, answer)
    } yield answers
  ...
  "DefaultProvideGoodsDescriptionController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.ProvideGoodsDescriptionBaseController.onSubmit(NormalMode)
