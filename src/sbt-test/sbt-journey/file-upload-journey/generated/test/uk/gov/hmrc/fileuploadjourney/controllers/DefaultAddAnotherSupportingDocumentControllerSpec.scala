...
class DefaultAddAnotherSupportingDocumentControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultAddAnotherSupportingDocumentFormProvider()
  ...
  private def makeController(
    ...
    formProvider: AddAnotherSupportingDocumentBaseFormProvider = mock[AddAnotherSupportingDocumentBaseFormProvider],
    view: AddAnotherSupportingDocumentView = mock[AddAnotherSupportingDocumentView]
  ) = new DefaultAddAnotherSupportingDocumentController(
  ...
  "DefaultAddAnotherSupportingDocumentController.onPageLoad(Yes, i)" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[AddAnotherSupportingDocumentView]
    ...
    val route = journeyRoutes.AddAnotherSupportingDocumentBaseController.onPageLoad(0, NormalMode)
    ...
    val submitRoute = journeyRoutes.AddAnotherSupportingDocumentBaseController.onSubmit(0, NormalMode)
  ...
  "DefaultAddAnotherSupportingDocumentController.onSubmit(Yes, i)" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: Choice) =>
    ...
    val route = journeyRoutes.AddAnotherSupportingDocumentBaseController.onSubmit(0, NormalMode)
  ...
  it should "redirect to journey recovery when a prerequisite answer is missing" in forAll(minSuccessful(5)) { (answer: Choice) =>
    val userAnswers = Success(UserAnswers("id"))
    val controller = makeController(userAnswers = userAnswers.toOption)
    val route = journeyRoutes.AddAnotherSupportingDocumentBaseController.onSubmit(0, NormalMode)
    val form = defaultFormProvider().fill(answer)
