...
class DefaultDataDomainControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultDataDomainFormProvider()
  ...
  private def makeController(
    ...
    formProvider: DataDomainBaseFormProvider = mock[DataDomainBaseFormProvider],
    view: DataDomainView = mock[DataDomainView]
  ) = new DefaultDataDomainController(
  ...
  "DefaultDataDomainController.onPageLoad(i)" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[DataDomainView]
    ...
    val route = journeyRoutes.DataDomainBaseController.onPageLoad(0, NormalMode)
    ...
    val submitRoute = journeyRoutes.DataDomainBaseController.onSubmit(0, NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(DataDomainPage(0), answer)
    } yield answers
  ...
  "DefaultDataDomainController.onSubmit(i)" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.DataDomainBaseController.onSubmit(0, NormalMode)
