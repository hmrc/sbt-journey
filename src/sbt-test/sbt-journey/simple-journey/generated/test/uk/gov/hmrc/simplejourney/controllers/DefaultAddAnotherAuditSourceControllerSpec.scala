...
import uk.gov.hmrc.simplejourney.forms.AddAnotherAuditSourceBaseFormProvider
...
import views.html.AddAnotherAuditSourceView

import scala.concurrent.{ExecutionContext,Future}
import scala.util.{Success,Try}

class DefaultAddAnotherAuditSourceControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultAddAnotherAuditSourceFormProvider()
  ...
  private def makeController(
    ...
    formProvider: AddAnotherAuditSourceBaseFormProvider = mock[AddAnotherAuditSourceBaseFormProvider],
    view: AddAnotherAuditSourceView = mock[AddAnotherAuditSourceView]
  ) = new DefaultAddAnotherAuditSourceController(
  ...
  "DefaultAddAnotherAuditSourceController.onPageLoad(i)" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[AddAnotherAuditSourceView]
    ...
    val route = journeyRoutes.AddAnotherAuditSourceBaseController.onPageLoad(0, NormalMode)
    ...
    val submitRoute = journeyRoutes.AddAnotherAuditSourceBaseController.onSubmit(0, NormalMode)
  ...
  "DefaultAddAnotherAuditSourceController.onSubmit(i)" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: Choice) =>
    ...
    val route = journeyRoutes.AddAnotherAuditSourceBaseController.onSubmit(0, NormalMode)
