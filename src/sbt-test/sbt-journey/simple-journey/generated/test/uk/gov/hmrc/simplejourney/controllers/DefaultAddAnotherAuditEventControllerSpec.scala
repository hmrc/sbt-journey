...
import uk.gov.hmrc.simplejourney.forms.AddAnotherAuditEventBaseFormProvider
...
class DefaultAddAnotherAuditEventControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  ...
  "DefaultAddAnotherAuditEventController.onSubmit(i, j)" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: Choice) =>
