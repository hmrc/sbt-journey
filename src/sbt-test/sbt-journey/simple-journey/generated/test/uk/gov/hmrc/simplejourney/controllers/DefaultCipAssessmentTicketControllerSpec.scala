...
class DefaultCipAssessmentTicketControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultCipAssessmentTicketFormProvider()
  ...
  private def makeController(
    ...
    formProvider: CipAssessmentTicketBaseFormProvider = mock[CipAssessmentTicketBaseFormProvider],
    view: CipAssessmentTicketView = mock[CipAssessmentTicketView]
  ) = new DefaultCipAssessmentTicketController(
  ...
  "DefaultCipAssessmentTicketController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[CipAssessmentTicketView]
    ...
    val route = journeyRoutes.CipAssessmentTicketBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.CipAssessmentTicketBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(CipAssessmentTicketPage, answer)
    } yield answers
  ...
  "DefaultCipAssessmentTicketController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.CipAssessmentTicketBaseController.onSubmit(NormalMode)
