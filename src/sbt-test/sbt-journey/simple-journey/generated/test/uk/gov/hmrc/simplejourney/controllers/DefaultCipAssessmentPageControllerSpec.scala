...
class DefaultCipAssessmentPageControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultCipAssessmentPageFormProvider()
  ...
  private def makeController(
    ...
    formProvider: CipAssessmentPageBaseFormProvider = mock[CipAssessmentPageBaseFormProvider],
    view: CipAssessmentPageView = mock[CipAssessmentPageView]
  ) = new DefaultCipAssessmentPageController(
  ...
  "DefaultCipAssessmentPageController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[CipAssessmentPageView]
    ...
    val route = journeyRoutes.CipAssessmentPageBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.CipAssessmentPageBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(CipAssessmentPagePage, answer)
    } yield answers
  ...
  "DefaultCipAssessmentPageController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.CipAssessmentPageBaseController.onSubmit(NormalMode)
