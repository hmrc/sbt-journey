...
class DefaultAuditSourceControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultAuditSourceFormProvider()
  ...
  private def makeController(
    ...
    formProvider: AuditSourceBaseFormProvider = mock[AuditSourceBaseFormProvider],
    view: AuditSourceView = mock[AuditSourceView]
  ) = new DefaultAuditSourceController(
  ...
  "DefaultAuditSourceController.onPageLoad(i)" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[AuditSourceView]
    ...
    val route = journeyRoutes.AuditSourceBaseController.onPageLoad(0, NormalMode)
    ...
    val submitRoute = journeyRoutes.AuditSourceBaseController.onSubmit(0, NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(AuditSourcePage(0), answer)
    } yield answers
  ...
  "DefaultAuditSourceController.onSubmit(i)" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.AuditSourceBaseController.onSubmit(0, NormalMode)
