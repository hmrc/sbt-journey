...
class DefaultServiceDescriptionControllerSpec extends AnyFlatSpec, Matchers, ScalaCheckPropertyChecks, RecoverMethods, TryValues, MockitoSugar, Generators {
  given system: ActorSystem = ActorSystem("test")
  given ExecutionContext    = system.dispatcher
  given Messages            = stubMessages()

  private val defaultFormProvider = new DefaultServiceDescriptionFormProvider()
  ...
  private def makeController(
    ...
    formProvider: ServiceDescriptionBaseFormProvider = mock[ServiceDescriptionBaseFormProvider],
    view: ServiceDescriptionView = mock[ServiceDescriptionView]
  ) = new DefaultServiceDescriptionController(
  ...
  "DefaultServiceDescriptionController.onPageLoad" should "return a 200 OK response containing the view HTML when everything is successful" in {
    val view = mock[ServiceDescriptionView]
    ...
    val route = journeyRoutes.ServiceDescriptionBaseController.onPageLoad(NormalMode)
    ...
    val submitRoute = journeyRoutes.ServiceDescriptionBaseController.onSubmit(NormalMode)
  ...
  it should "pre-fill the form if the user has already supplied an answer" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val userAnswers = for {
      ...
      answers <- answers.set(ServiceDescriptionPage, answer)
    } yield answers
  ...
  "DefaultServiceDescriptionController.onSubmit" should "redirect to the next page when everything is successful" in forAll(minSuccessful(5)) { (answer: String) =>
    ...
    val route = journeyRoutes.ServiceDescriptionBaseController.onSubmit(NormalMode)
