...
class UpscanConnectorSpec extends AnyFlatSpec with Matchers {
  ...
  "UpscanConnector.renderSpec" should "render a template for UpscanConnectorSpec relative to the base package" in {
    UpscanConnector.renderSpec("sbt-journey-test", basePackage) shouldBe
      s"""package uk.gov.hmrc.sbtjourneytest.connectors
        |
        ...
        |      fields = Map(
        |        "success_action_redirect" -> s"http://localhost:$$wireMockPort/sbt-journey-test/check-your-answers?key=$$fileReference",
        |        "x-amz-credential"                    -> "ASIAxxxxxxxxx/20180202/eu-west-2/s3/aws4_request",
        |        "x-amz-meta-upscan-initiate-response" -> formatter.format(responseTime),
        |        "x-amz-meta-original-filename"        -> "$${filename}",
        |        "x-amz-algorithm"                     -> "AWS4-HMAC-SHA256",
        |        "x-amz-signature"                     -> "xxxx",
        |        "error_action_redirect"   -> "http://localhost:$$wireMockPort/sbt-journey-test/journey-recovery",
        |        "x-amz-meta-session-id"   -> sessionId,
        |        "x-amz-meta-callback-url" -> "http://localhost:$$wireMockPort/sbt-journey-test/",
        ...
        |    app.RoutesPrefix.setPrefix("/sbt-journey-test")
        |  }
        |
        ...
        |        .withHeader(HeaderNames.USER_AGENT, equalTo("sbt-journey-test"))
        |        .withRequestBody(equalToJson(s\"\"\"{
        |        |  "callbackUrl": "$$wireMockUrl/sbt-journey-test",
        |        |  "successRedirect": "$$wireMockUrl/sbt-journey-test/check-your-answers",
        |        |  "errorRedirect": "$$wireMockUrl/sbt-journey-test/there-is-a-problem"
        |        |}\"\"\".stripMargin))
        |        .willReturn(
        |          okJson(
        |            Json.stringify(
