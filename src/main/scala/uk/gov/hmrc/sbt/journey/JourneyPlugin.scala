...
object JourneyPlugin extends AutoPlugin {
  ...
  private[journey] def deserialiseJourneyPart(
    ...
    value match {
      ...
      case obj: ConfigObject if obj.containsKey("if") || obj.containsKey("then") =>
        ...
        val answerType = Option(pages(choicePage).answerType)
        val modelType  = answerType.flatMap(_.typeName).flatMap(models.get)
        ...
        modelType match {
          ...
          case _ =>
            if (
              ...
              errors += problem(
                obj.origin(),
                "Expected a choice page with a Boolean or enum answerType"
              )
            }
  ...
  private[journey] def generateJourneyTestFiles(
    ...
    IO.write(journeyGeneratorFile, journeyGenerators.render(basePackage, hasFileUploadPage))
