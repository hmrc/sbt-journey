...
object FormatTraits {
  def extendsClause(importedSymbols: Map[List[String], Set[String]], withTraits: QualifiedName*): String = {
    val formatTraits =
      if (Imports.usesHmrcMongoJavaTime(importedSymbols))
        List("MongoJavatimeFormats.Implicits")
      else
        List.empty

    val allTraits = withTraits.map(_.parts.last) ++ formatTraits

    if (allTraits.isEmpty) "" else allTraits.mkString("extends ", ", ", " ")
  }

  def enumFormats(basePackage: QualifiedName): String = {
